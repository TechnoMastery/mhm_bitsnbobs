...
public class MysteriousAltarBlockEntity extends AbstractMenuBlockEntity {
    ...
    public MysteriousAltarBlockEntity(BlockPos pPos, BlockState pBlockState) {
        super(ModBlockEntities.MYSTERIOUS_MAGIC_BE.get(), pPos, pBlockState, new DelegatingItemStackHandler(7) {
            @Override
            public boolean isItemValid(int slot, @NotNull ItemStack stack) {
                if (INPUT_SLOTS.contains(slot))  return true;
                if (slot == FUEL_SLOT) return stack.is(ModTags.Items.MAGIC_FUELS);
                return false;
            }
        });
